...
| 3 | JAN（EAN-13）バーコードを縦向きのまま提示すると読み取れない場合がある。原因は `@zxing/browser`（0.2.1、2026-09時点で最新）の `HTMLCanvasElementLuminanceSource.rotate()` が回転後の画素データに対して `width`/`height` を更新し直さないバグで、1次元コード用の `TRY_HARDER`（90度回転しての再試行）ヒントが実質的に機能しない。QRコード・GS1 DataMatrixはファインダーパターンで自己回転検出するため影響を受けない（`inport_qr.vue`） | 縦向きに提示すると検出されず、横向きに持ち直す必要がある | スキャン画面に「JANバーコード（縦縞模様のバーコード）は横向きにしてください」と案内文を表示（`inport_qr.vue`実装済み）。恒久対応（ライブラリのpatch-package修正、置き換え等）は7章7参照 |
...
| 7 | 縦向きJANバーコードが読み取れない `@zxing/browser` のバグ（`HTMLCanvasElementLuminanceSource.rotate()` の `width`/`height` 不整合）への恒久対応の要否確定。現状はUI案内文のみで対応。恒久対応案：patch-packageでのライブラリ修正、`window.BarcodeDetector` 等ブラウザ標準APIへの置き換え | 実装フェーズ以降 | 6.8 制限3 |
