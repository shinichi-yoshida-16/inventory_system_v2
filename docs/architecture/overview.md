...
| `server/utils/alert.ts` | ロジック層 | 通知先設定への在庫アラートメール送信・マージ送信（FR-09） |
...
| 権限エラー | API層の管理者判定 | `PERMISSION_DENIED` | HTTP 403。非管理者が管理者専用API（閾値・廃番・通知先・時差更新）を呼んだ場合。画面遷移しない（D-05 の導線は元々非管理者に非表示） |
...
- D-05 の時差更新ボタン（管理者のみ、蓄積データがある場合に活性化）で `POST /api/deferred-sync` を呼び、`pending/` 直下（デリミタ `/`。`pending/alerts/` は除外）を名前順に適用 → 成功分を削除。全件成功でユーザに完了通知、失敗が残れば蓄積データを保持して再実施を促す（3-12）。
