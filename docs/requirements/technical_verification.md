...
- 運用制約：**「コードスキャン画面は必ず Safari / Chrome 等の通常ブラウザで開く」**をルール化する（取扱説明書に反映予定。overview.md 6.5 と整合）。アプリ内ブラウザ検知時の案内メッセージは実装済み（`inport_qr.vue`）。カメラを使わない手入力フォールバックの追加は引き続き検討余地（overview.md 7章 #6）。
